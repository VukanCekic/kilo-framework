...
5. [Mode 1: Architect (Planning & Review)](#mode-1-architect)
6. [Mode 2: Workhorse (Implementation)](#mode-2-workhorse)
7. [Example Flows](#example-flows)
8. [Session Lifecycle Commands](#session-lifecycle-commands)
...
.kilo/
├── agents/
│   ├── 01_architect.md      # Planning & review agent
│   └── 02_workhorse.md      # Implementation agent
└── rules/
...
The framework ships with two specialized agents orchestrated by the primary mode.

| Agent | Purpose | Permissions |
|-------|---------|-------------|
| **01_architect** | Analyzes code, plans architecture, writes docs, reviews PRs | Ask before edit/bash |
| **02_workhorse** | Executes defined tasks, writes code, runs tests, refactors | Full edit + bash |

The orchestrator (`kilo.jsonc`) routes tasks:
- By default, tasks are **denied** unless explicitly delegated to an allowed agent.
- The architect and workhorse are the only two allowed subagents.
...
- Proposes architectural changes with tradeoff analysis.
...
### What It Does
- Receives highly specific technical requirements.
...
### Flow A: New Feature (Architect → Workhorse)

A complete lifecycle from idea to shipped feature, using both agents.

#### Step 1: Start the session

```
User: [Click "Continue Project Task"]
AI:  Loaded previous session summary. Project: e-commerce API.
     Active tasks: checkout module 60% complete.
```

#### Step 2: Delegate to Architect for planning
...
#### Step 4: Delegate to Workhorse for implementation
...
AI:  No files modified. memory-bank/ was not touched.
```
...
- **Architect** for "should we?" and "how should we?"
- **Workhorse** for "do it" and "fix it"

Passing implementation work to the architect will slow you down with approval prompts. Passing open-ended design work to the workhorse may result in premature code without proper analysis.
...
| Plan before coding | Use `@01_architect` |
| Write/fix code | Use `@02_workhorse` |
| Debug a stubborn issue | Tell workhorse to follow `debug.md` protocol |
| Update project status | Ask the AI to update `memory-bank/project-state.md` |
| Track tasks | Ask the AI to update `memory-bank/tasks.md` |
